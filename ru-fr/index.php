...
$APPLICATION->SetPageProperty('title', 'Продажа недвижимости в России и за рубежом - более '. $getCount .' объявлений на Fodyo.com 🏘');

$APPLICATION->SetPageProperty('description', 'База объявлений недвижимости по всему миру на Fodyo.com. Удобный поиск ✅ Новостройки, квартиры, таунхаусы ✅ Объявления  от собственников, застройщиков и агентств недвижимости ✅ Портал недвижимости Fodyo.com предлагает более '. $getCount .' объявлений по России.')
?>
...
<div class="main-categories">
    <div class="flex-row">
        ...
        if (CModule::IncludeModule("iblock")){
            $getSections = GetAllSectionIn(4, 1314, array('ACTIVE' => 'Y', 'UF_SHOW_MAIN_LIST' => '1', 'DEPTH_LEVEL' => '3'), array('ID', 'NAME', 'PICTURE', 'UF_NAME_RU', 'SECTION_PAGE_URL', 'DEPTH_LEVEL'));
                //echo "<pre>"; print_r($getSections); echo "</pre>";
        }
        $getSect = CIBlockSection::GetList(
            Array("SORT"=>"ASC"),
            Array('ID' => 1314, 'IBLOCK_ID' => 4),
            false,
            array('ID', 'NAME', 'PICTURE', 'UF_NAME_RU', 'SECTION_PAGE_URL', 'DEPTH_LEVEL', 'UF_HREF_ISO'),
            false
        );
        while ($getItem = $getSect->GetNext()) {
            $isoLang = $getItem['UF_HREF_ISO'];
            //echo "<pre>"; print_r($getItem); echo "</pre>";
        }
        foreach ($getSections as $key => $value) {
            //echo "<pre>"; print_r($value['SECTION_PAGE_URL']); echo "</pre>";
            $value['SECTION_PAGE_URL'] = '/'.strtolower(LANGUAGE_ID).'-'.$isoLang.str_replace('ru/', '',str_replace('/russia', '', $value['SECTION_PAGE_URL']));
            if($key == 0){
                ?>
                <div class="stolb">
