...
$APPLICATION->SetPageProperty('title', 'Продажа недвижимости в США и за рубежом - более '. $getCount .' объявлений на Fodyo.com 🏘');

$APPLICATION->SetPageProperty('description', 'База объявлений недвижимости по всему миру на Fodyo.com. Удобный поиск ✅ Новостройки, квартиры, таунхаусы ✅ Объявления  от собственников, застройщиков и агентств недвижимости ✅ Портал недвижимости Fodyo.com предлагает более '. $getCount .' объявлений по Америке.')
?>
...
<div class="main-categories">
    <div class="flex-row">
        ...
        if (CModule::IncludeModule("iblock")){
            $getSections = GetAllSectionIn(4, 1301, array('ACTIVE' => 'Y', 'UF_SHOW_MAIN_LIST' => '1', 'DEPTH_LEVEL' => '3'), array('ID', 'NAME', 'PICTURE', 'UF_NAME_RU', 'SECTION_PAGE_URL', 'DEPTH_LEVEL'));
                //echo "<pre>"; print_r($getSections); echo "</pre>";
        }
        $getSect = CIBlockSection::GetList(
            Array("SORT"=>"ASC"),
            Array('ID' => 1301, 'IBLOCK_ID' => 4),
            false,
            array('ID', 'NAME', 'PICTURE', 'UF_NAME_RU', 'SECTION_PAGE_URL', 'DEPTH_LEVEL', 'UF_HREF_ISO'),
            false
        );
        if($fetchSect = $getSect->GetNext()){
            $isoLang = $fetchSect['UF_HREF_ISO'];
        }
        foreach ($getSections as $key => $value) {
            //echo "<pre>"; print_r($value['SECTION_PAGE_URL']); echo "</pre>";
            $value['SECTION_PAGE_URL'] = '/'.strtolower(LANGUAGE_ID).'-'.$isoLang.str_replace('ru/', '',str_replace('/usa', '', $value['SECTION_PAGE_URL']));
            if($key == 0){
...
<?/*<div class="other-cities">
    <div class="max-width">
        ...
        <div class="flex-cities">
            ...
            $getSect = CIBlockSection::GetList(
                Array("SORT"=>"ASC"),
                Array('UF_SHOW_MAIN_LIST' => '1', 'IBLOCK_ID' => 4, 'DEPTH_LEVEL'),
                false,
...
<div class="other-cities">
    <div class="max-width">
        <div class="block-title">
            <span>Другие страны на Фодио</span>
        </div>
        <ul class="flex-countries">
            <?
            $getSect = CIBlockSection::GetList(
                Array("SORT"=>"ASC"),
                Array('DEPTH_LEVEL' => '2', 'IBLOCK_ID' => 4, '!ID' => 745),
                false,
                array('ID', 'NAME', 'PICTURE', 'SECTION_PAGE_URL', 'DEPTH_LEVEL', 'UF_FLAG', 'UF_NAME_'.strtoupper(LANGUAGE_ID), 'UF_HREF_ISO'),
                false
            );
            while ($nextSection = $getSect->GetNext()) {
                ...
                ?>
                <li><img src="<?=$file['src']?>" height="15" width="25"><a href="<?=$nextSection['SECTION_PAGE_URL']?>"><span><?=$nextSection['UF_NAME_'.strtoupper(LANGUAGE_ID)]?></span></a></li>
                <?
            }
            ?>
        </ul>
...
<div class="news-main">
    <div class="block-title">
        ...
        <a class="read_all_news" href="/ru/articles/">читать все статьи</a>
    </div>
...
<div class="background-gray">
    <div class="max-width">
        <div class="about-main">
            <div class="block-title">
                <span class="company-title">О портале</span>
            </div>
            ...
            <a class="read_more" href="/ru/about/">читать далее</a>
        </div>
    </div>
</div>
