...
if( stristr($APPLICATION->GetCurDir(), 'neighborhood') ){
    ...
    if($fetchTest = $testRes -> GetNext()){
        $getSections = GetAllSectionIn(4, $fetchTest['IBLOCK_SECTION_ID'], array( "!PICTURE" => false), array('ID', 'NAME', 'IBLOCK_ID', 'PICTURE', 'UF_NAME_RU', 'UF_IS_PAGE', 'SECTION_PAGE_URL', 'DEPTH_LEVEL'));
        ...
        if (!empty($arResult['ITEMS']) && !empty($arResult['ITEM_ROWS']))
        {
            ...
            foreach ($arResult['ITEM_ROWS'] as $rowData)
            {
                ...
                <div class="items-preview-product flex-row" data-entity="items-row">
                    ...
                    while($rowCount <= 3){
                        $rowCount++;
                        echo '<div class="preview-product-item"></div>';
                    }
                    ?>
                </div>
                <?
...
if(LANGUAGE_ID == 'en'){
    ...
    $APPLICATION->SetPageProperty("title", 'Buy an apartment in '. $arResult['NAME'] .', 🏢 apartments for sale in the capital, ads and prices on Fodyo.com');

    $APPLICATION->SetPageProperty("description", 'A complete collection of allapartments in '. $arResult['NAME'] .' at a low price. The 👉🏻 Fodyo.com 👈🏻 portal is useful for those who are interested in information on apartments in '. $arResult['NAME'] .'. Here are apartments in '. $arResult['NAME'] .' for sale on favorable terms. ✅');
