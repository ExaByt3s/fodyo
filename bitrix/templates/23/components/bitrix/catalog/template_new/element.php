...
if($arResult['VARIABLES']['ELEMENT_CODE']){
    $getItemId = CIBlockElement::GetList(array('SORT'=>'ASC'), array('CODE' => $arResult['VARIABLES']['ELEMENT_CODE']));
    if($fetchItemId = $getItemId -> GetNext()){
        $itemId = $fetchItemId['ID'];
    }
}else{
    $itemId = $arResult['VARIABLES']['ELEMENT_ID'];
}

$db_old_groups = CIBlockElement::GetElementGroups($itemId, true);
...
if($ufHref != $getSectionCode || !$arResult['VARIABLES']['SECTION_CODE_PATH']){
