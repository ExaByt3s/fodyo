...
if($arResult["VARIABLES"]["SECTION_ID"] == 1112 || $arResult['VARIABLES']['SECTION_CODE_PATH'] == 'single-family-homes'){
    //parse_url($_SERVER['REQUEST_URI']);
    //echo $_SERVER['REQUEST_URI'];
    $explo = explode('?', $_SERVER['REQUEST_URI']);

    $explo = explode('/', $explo[0]);
    //echo "<pre>"; print_r($explo); echo "</pre>";
    if($explo[1] == 'single-family-homes'){
        $getSectionCode = 'us';
    }
    elseif($explo[1] != 'bitrix'){
        $explo = explode('-', $explo[1]);
        if(empty($explo[1]) || $explo[1] == '' || $explo[0] == "single-family-homes"){
            $getSectionCode = 'us';
        }else{
            $getSectionCode = $explo[1];
        }
    }
    //echo "<pre>"; print_r($getSectionCode); echo "</pre>";
    ...
    if($fetchSection = $getSectList -> GetNext()){
        ...
        include $_SERVER['DOCUMENT_ROOT'].'/404.php';
        die();
        //echo "<pre>"; print($explodeSec); echo "</pre>";
