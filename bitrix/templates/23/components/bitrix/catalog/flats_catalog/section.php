...
$getItem = CIBlockSection::GetList(array('SORT'=>'ASC'), array('UF_HREF_ISO' => ISO, 'IBLOCK_ID' => $arParams['IBLOCK_ID']));
global $USER;
if($USER->isAdmin()){
	$fetchItem = $getItem->Fetch();
	$getParentId = $fetchItem['ID'];
	//echo "<pre>"; print_r($fetchItem); echo "</pre>";
}

$explodeSec = explode('/', $arResult["VARIABLES"]["SECTION_CODE_PATH"]);

if($arResult["VARIABLES"]["SECTION_ID"] == $getParentId || $arResult['VARIABLES']['SECTION_CODE_PATH'] == 'condos'){
    ...
    elseif($explo[1] != 'bitrix'){
        ...
        if(empty($explo[1]) || $explo[1] == '' || $explo[0] == "condos"){
            $getSectionCode = 'us';
        }else{
    ...
    if(stristr('neighborhood', $_SERVER['REQUEST_URI'])){
        $getSectList = CIBlockSection::GetList(array('SORT'=>'ASC'), array('IBLOCK_ID' => 4, 'UF_HREF_ISO' => $getSectionCode),false,array('ID','NAME', 'IBLOCK_ID', 'UF_HREF_ISO'));
    }else{
        $getSectList = CIBlockSection::GetList(array('SORT'=>'ASC'), array('IBLOCK_ID' => $arParams['IBLOCK_ID'], 'UF_HREF_ISO' => $getSectionCode),false,array('ID','NAME', 'IBLOCK_ID', 'UF_HREF_ISO'));
    }

    while($fetchSection = $getSectList -> GetNext()){
        
        
        $arResult["VARIABLES"]["SECTION_ID"] = $fetchSection['ID'];
        //$arResult["VARIABLES"]["SECTION_CODE"] = $fetchSection['CODE'];
        //$arResult["VARIABLES"]["SECTION_CODE_PATH"] .= '/'.$fetchSection['CODE'];
    }
    ...
    $getSectList = CIBlockSection::GetList(array('SORT'=>'ASC'), array('IBLOCK_ID' => $arParams['IBLOCK_ID'], 'CODE' => $explodeSec[count($explodeSec) - 1]),false,array('ID','NAME', 'IBLOCK_ID', 'UF_HREF_ISO'));
    ...
    if($fetchSection = $getSectList -> GetNext()){
        ...
        if($ufHref != $getSectionCode){
            ...
            if(stristr($arResult['VARIABLES']['SECTION_CODE_PATH'], 'price')){
            ...
            }elseif( stristr($arResult['VARIABLES']['SECTION_CODE_PATH'], 'studios') || stristr($arResult['VARIABLES']['SECTION_CODE_PATH'], 'beds') || stristr($arResult['VARIABLES']['SECTION_CODE_PATH'], 'penthouses') ){
                ...
                include($_SERVER["DOCUMENT_ROOT"]."/".$this->GetFolder()."/section_beds.php");
            }else{
                $arResult["VARIABLES"]["SECTION_ID"] = $fetchSection['ID'];
