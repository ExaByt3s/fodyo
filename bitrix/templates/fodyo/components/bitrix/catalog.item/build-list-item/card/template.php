...
<div class="build-item-bg" style="background-image: url('http://ru-dn.ru/wp-content/uploads/2018/10/новостройки.jpg');"></div>
    <div class="build-item-prop">
      <div class="build-status fl-row">
        <div>Девелопер</div>
        <div>Строится</div>
      </div>
      <h3>Комплекс апартаментов ОДП (Орехово.Дом у парка)</h3>
      <p>Россия, Москва, СВАО</p>
      <p class="build-near-metro">
        <img src="/images/logo_mos_metro.png">
        <span>Домодедовская</span>
        <img src="/images/logo_mos_metro.png" alt="здесь должна быть иконка ходьбы или машины. Слава формату xcf">
        2 мин.
      </p>
      <div class="room-quantity">
        <div>
          <span>Студия</span>
          <span>от 35.94 м2</span>
        </div>
        <span>7 324 324Р</span>
      </div>
      <div class="room-quantity">
        <div>
          <span>1-комн.</span>
...
<div class="build-item-prop">
  ...
  <? endif; ?>
  <?
  //$countersMinSquare = array('','','','');
  //
  //echo "<pre>";print_r($item['OFFERS']);echo "</pre>";
  foreach ($item['OFFERS'] as $key => $offer) {

    //echo "<pre>";print_r($offer['NAME']);echo "</pre>";

    if( is_array($countersMinSquare[ $name ]) && $offer['PROPERTIES']['AREA_SPACE']['VALUE'] < $countersMinSquare[ $name ]['MIN_SQUARE']){

      $name = $offer['PROPERTIES']['SKU_TYPE_OF_ITEM']['VALUE_XML_ID'];
      $countersMinSquare[ $name ]['MIN_SQUARE'] = $offer['PROPERTIES']['AREA_SPACE']['VALUE'];
      $countersMinSquare[ $name ]['PRICE_ARR'] = $offer['ITEM_PRICES'];

    }else{

      $name = $offer['PROPERTIES']['SKU_TYPE_OF_ITEM']['VALUE_XML_ID'];
      $countersMinSquare[ $name ]['MIN_SQUARE'] = $offer['PROPERTIES']['AREA_SPACE']['VALUE'];
      $countersMinSquare[ $name ]['NAME'] = $offer['PROPERTIES']['SKU_TYPE_OF_ITEM']['VALUE_ENUM'];
      $countersMinSquare[ $name ]['MEASURE'] = 'м2';
      $countersMinSquare[ $name ]['PRICE_ARR'] = $offer['ITEM_PRICES'];

      //$countersMinSquare[ $name ]['PRCIE'] = $offer['ITEM_PRICES'][0]['PRINT_RATIO_PRICE']
  ...
  foreach ($countersMinSquare as $key => $value) {
    //echo "<pre>";print_r($value['NAME']);echo "</pre>";
  }
  $counter=0;
  if(is_array($countersMinSquare['STUDY'])){
    $counter++;
    ?>
    <div class="room-quantity">
      <div>
        <span><?=$countersMinSquare['STUDY']['NAME']?></span>
        <span><?echo "от ".$countersMinSquare['STUDY']['MIN_SQUARE'].' м2';?></span>
      </div>
      <span>
        <?
        foreach ($countersMinSquare[ 'STUDY' ]['PRICE_ARR'] as $key => $price) {

          if($print_price != '' && $print_price > $price['RATIO_PRICE']){
            $print_price = $price['RATIO_PRICE'];
            //$print_price = $price['PRINT_RATIO_PRICE'];
          }
          else{
            $print_price = $price['RATIO_PRICE'];
            //$print_price = $price['PRINT_RATIO_PRICE'];
        ...
        echo number_format($print_price*$countersMinSquare['STUDY']['MIN_SQUARE'], 0, '', ' ').'Р';
        unset($print_price);
        ?>
      </span>
    </div>
  ...
  if(is_array($countersMinSquare['1_ROOMED'])){
    $counter++;
    ?>
    <div class="room-quantity">
      <div>
        <span><?=$countersMinSquare['1_ROOMED']['NAME']?></span>
        <span><?echo "от ".$countersMinSquare['1_ROOMED']['MIN_SQUARE'].' м2';?></span>
      </div>
      <span>
        <?
        foreach ($countersMinSquare[ '1_ROOMED' ]['PRICE_ARR'] as $key => $price) {
        ...
        echo number_format($print_price*$countersMinSquare['1_ROOMED']['MIN_SQUARE'], 0, '', ' ').'Р';
  ...
  if(is_array($countersMinSquare['2_ROOMED'])){
    $counter++;
    ?>
    <div class="room-quantity">
      <div>
        <span><?=$countersMinSquare['2_ROOMED']['NAME']?></span>
        <span><?echo "от ".$countersMinSquare['2_ROOMED']['MIN_SQUARE'].' м2';?></span>
      </div>
      <span>
        <?
        foreach ($countersMinSquare[ '2_ROOMED' ]['PRICE_ARR'] as $key => $price) {
        ...
        echo number_format($print_price*$countersMinSquare['2_ROOMED']['MIN_SQUARE'], 0, '', ' ').'Р';
  ...
  if(is_array($countersMinSquare['3_ROOMED'])){
    $counter++;
    ?>
    <div class="room-quantity">
      <div>
        <span><?=$countersMinSquare['3_ROOMED']['NAME']?></span>
        <span><?echo "от ".$countersMinSquare['3_ROOMED']['MIN_SQUARE'].' м2';?></span>
      </div>
      <span>
        <?
        foreach ($countersMinSquare[ '3_ROOMED' ]['PRICE_ARR'] as $key => $price) {
        ...
        echo number_format($print_price*$countersMinSquare['3_ROOMED']['MIN_SQUARE'], 0, '', ' ').'Р';
  ...
  if(is_array($countersMinSquare['4_ROOMED']) && $counter < 4){
    $counter++;
    ?>
    <div class="room-quantity">
      <div>
        <span><?=$countersMinSquare['4_ROOMED']['NAME']?></span>
        <span><?echo "от ".$countersMinSquare['4_ROOMED']['MIN_SQUARE'].' м2';?></span>
      </div>
      <span>
        <?
        foreach ($countersMinSquare[ '4_ROOMED' ]['PRICE_ARR'] as $key => $price) {
        ...
        echo number_format($print_price*$countersMinSquare['4_ROOMED']['MIN_SQUARE'], 0, '', ' ').'Р';
